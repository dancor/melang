...
main = do
    ...
    prevPronMap <- Map.fromList . map (second Set.singleton) .
        map (breakOnCh '\t') . filter (not . ("\\" `DT.isInfixOf`)) .
        DT.lines <$> DTI.readFile pronF
    ...
    let pronMap = dedupeCaps .
        ...
        doGoogWd x = DT.concat [gWd x, "\t", pronStr]
          where
            ...
            pronByChr = maybe "?" handleEachChrPron eachChrPronMby
            handleEachChrPron = DT.intercalate " | " .
                map DT.concat . sequence . map Set.toList
            eachChrPronMby = sequence
